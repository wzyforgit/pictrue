...
/*打开24位BMP图片*/
dm24* getDataMap_24bit(FILE* bmpFile, picHead *h)
{
	GETSIZE();
	dm24 *dataMap;
	/*申请存储图像数据的内存*/
...
/*打开8位BMP图片*/
dm8* getDataMap_8bit(FILE* bmpFile, picHead *h)
{
...
/*24位BMP图片存储*/
void savePicture_24bit(char* fileName, picHead *h, dm24 *dataMap)
{
	if (dataMap == NULL)
	{
		printf("未传入像素数据!\n");
...
/*8位BMP图片存储*/
void savePicture_8bit(char* fileName, picHead *h, dm8 *dataMap)
{
	if (dataMap == NULL)
	{
		printf("未传入像素数据!\n");
